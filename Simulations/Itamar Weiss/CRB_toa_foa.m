function [CRB_toa, CRB_foa] = CRB_toa_foa(Fc,c, delayedSignal, delayedSignalDot, N, Fs, b, rTransmitter,v, rReceiverMat,SNRdB)
...
CRB_foa = zeros(1,L);
for l=1:L
dMldTl = -b(l)*A(:,:,l)*C*sDotl(:,l);
dMldFl = 2*pi*1i*Ts*b(l)*Ntilde*A(:,:,l)*C*sl(:,l);

CRB_toa(l) = sigmal(1)^2/(2*dMldTl'*dMldTl);
CRB_foa(l) = sigmal(1)^2/(2*dMldFl'*dMldFl);
end
