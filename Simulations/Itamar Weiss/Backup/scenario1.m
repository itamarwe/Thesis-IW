...
scenarioResults = zeros(size(scenarios,1),4);
...
for scenario = 1:size(scenarios,1)
    ...
    R = scenarios(scenario,5);% [m] Receivers Distance From the axis center


    %Transmitter Position, Velocity, Nominal Frequency, Bandwidth
    rTransmitter = [scenarios(scenario,6) scenarios(scenario,7)]; %Transmitter position [x,y] [m]
    v = [scenarios(scenario,8) scenarios(scenario,9)];%Transmitter velocity [vx, vy] [m/s]
    Fc = scenarios(scenario,10); % Carrier Frequency[Hz]
    B = scenarios(scenario,11);

    SNRdB = scenarios(scenario,12);
    receiverGeometry = scenarios(scenario,13);
    method = scenarios(scenario,14);
    estimations = scenarios(scenario,15);
    ...
    for estimation = 1:estimations
        ...
        [originalSignal, cleanReceivedSignal, sNorm2, sDotNorm2 ,imCorrssDot] = CreateRxSignal(rTransmitter, v, C, Fc, rReceiverMat,B,Fs, N+maxGridSampleDelay);
        receivedSignal = addNoise(cleanReceivedSignal,SNRdB);

        %***********************************
        %Cramer-Rao Lower Bound Calculation
        %CRBMat = CRB(Fc,C,N/Fs,B, SNRdB ,sNorm2, sDotNorm2, imCorrssDot, rTransmitter,v, rReceiverMat);
        %***********************************
        ...
        pScale =  50;
        vScale = 50;
        gridX = [linspace(-pScale/2, pScale/2,5)]+rTransmitter(1)+ (rand-.5)*10;
        gridY = [linspace(-pScale/2, pScale/2,5)]+rTransmitter(2)+(rand-.5)*10;
        gridVx = [linspace(-vScale/2, vScale/2,5)]+v(1)+(rand-.5)*10;
        gridVy = [linspace(-vScale/2, vScale/2,5)]+v(2)+(rand-.5)*10;
        
        while pScale > .1
                 [x y vx vy] = GridSearch5(gridX, gridY, gridVx, gridVy, originalSignal, rReceiverMat, receivedSignal, Fc, C, Fs, N, method);
%                [EmitPosEst_DPD,Xest_DPD,Yest_DPD]  = DPD_Known_Signals(time_vec,Xsearch_DPD,Ysearch_DPD,ReceiverPos,ReceiverVel,K,L,Q,fc,c,r_vec_t,CF1,sig);
    ...
    scenarioResults(scenario,:) = sqrt(mean(estimationResults.^2));
