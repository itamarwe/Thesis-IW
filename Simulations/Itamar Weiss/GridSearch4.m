function [x,y,vx,vy] = GridSearch4(gridX,gridY,gridVx,gridVy, originalSignal, rReceiverMat, receivedSignal, Fc, C, Fs, N, method)
    ...
    for gY = gridY
     ...
     %           end
               ...
               % sampleMaxDelay = max(sampleDelay);
                ...
                V = zeros(N,L);
                for l=1:L
                   % vk = dopplerFixedSignal(l,:)*F;
                   vk = fft(dopplerFixedSignal(l,:));
                   vk = vk.*exp(2*pi*i/N*k*sampleDelay(l));
                    vl = ifft(vk);
                    V(:,l) = vl(1:N);
                ...
                if (method==2) %Unknown Signals
                    ...
                    cost = max(real(eig(Qtilda)));             
                elseif (method==1) %Known Signals
                    cost = real(originalSignal(1:N)*V*V'*originalSignal(1:N)');
                end
                if (cost>maxCost)
                    x = gX;
                    y = gY;
