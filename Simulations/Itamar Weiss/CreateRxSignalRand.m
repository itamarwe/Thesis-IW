...
function [originalSignal, cleanReceivedSignal, sNorm2, sDotNorm2,imCorrssDot,delayedSignal, delayedSignalDot,b] = CreateRxSignalRand(rTransmitter, v, C, Fc, rReceiverMat,B,Fs,N)
%******************************************************
...
if (STEPSIGNAL)
    ...
    m = floor(N/2);
    ak = ak.*exp(-2*pi*1i/N*k*m);
else
    ak = exp(rand(1,N)*2*pi*1i);
    %ak=(rand(1,N)+1i*rand(1,N))/sqrt(2);
end
...
%    hzDopplerShift = Fc*miu;
    ...
    vA = exp(2*pi*1i*hzDopplerShift*(1:N)/Fs);
...
for l=1:L
    hzDopplerShift = -(Fc/C)*v*(rDiffMat(l,:)'/rDistances(l));
    cleanReceivedSignal(l,:)=time_freq_shift(originalSignal,Fs,delay(l)*1e-6,hzDopplerShift);
end
