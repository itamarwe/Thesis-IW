function [EmitPosEst_ST,Xest_ST,Yest_ST]  =  STPosEst(FD,TD,XsearchST,YsearchST,K,L,ReceiverPos,ReceiverVel,fc,c,CRB_f,CRB_tau, CF1)       
...
for x_ind  =   1:length(XsearchST)
      for y_ind  =   1:length(YsearchST)
          ...
          TOAdiff = diff(TOA_tmp);
          Dopplerdiff = diff(Doppler_shift_tmp);
          
          CF_ST(x_ind,y_ind) = w_toa*sum((TOAdiff - TD').^2) + w_dop*sum(( Dopplerdiff - FD'  ).^2)  ;
