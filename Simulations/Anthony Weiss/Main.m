...
dist_mult = 1e3;
EmitterPos  = [4500 3000]; % [[1+5*rand,1+5*rand]*dist_mult;
...
ylabel('Meters')
axis equal
legend('1st Platform','2nd Platform','Transmitter')
drawnow
...
for k = 1:K
    ...
    for ell = 1:L
        ...
        int=int./norm(int);
        int0 = amplitudes*cos( 2*pi*(f1')*(time_vec-TOA(ell,k)-T0/2)) ;
        int0 = int0./norm(int0);
        Delayed_sig(:,k,ell)=(int.').*exp(1i*2*pi*DD*time_vec');
        Delayed_sig0(:,k,ell)=(int0.').*exp(1i*2*pi*Doppler_shift(ell,k)*time_vec');
    end;%ell
end;%k

%[Poserr_DPD_K,Poserr_ST_K,Bias_vec_DPD_K,Bias_vec_ST_K,CRBp_Std_K] = known_signals_routine(SNRdB,Nexp,Ns,K,L,Q,fc,c,T0,...
 %   time_vec,amplitudes,TOA,Doppler_shift,sigma2Vec,EmitterPos,ReceiverPos,ReceiverVel,CF1,CF11,f1,DPDRun,STRun,max_dop,Delayed_sig,Delayed_sig0,sig);
