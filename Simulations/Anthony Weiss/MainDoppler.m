...
close all;
clc;
CF1=1; %show cost function
Known_Unknown_Sig   = 0; % 1- Known Signal, 0 - Unknown Signals
c = 3e8;                                               % Speed of light - 3*10^2 m/s
Q = 1;                                                  % Number of emitters
K = 3; %5                                         % Number of interception intervals
L = 2;%3;                                              % Number of receivers
SNRdB =  (5:5:45) ;  %ajw
ReceiverSpeed = 1e2;                      % receiver velocity (identical to all)
dist_mult = 1e3;
EmitterPos  = [2000 4000]; % [[1+5*rand,1+5*rand]*dist_mult;
...
Fs=Fsc/10;
Tsc = 1/Fsc;
Nsc = 2^10 ;   % - Number of samples in observation (after sampling the observation)


%Ns = NsVec(ns_ind);
time_vec = (0:Nsc-1)*Tsc;
freq_vec = (0:Nsc-1)*(Fsc/Nsc);
...
ReceiverPos(:,1,2) =     (1:K)'*dist_mult+500;
...
r_vec_t = zeros(Nsc,K,L);
r_vec_f = zeros(Nsc,K,L);
r_vec_f_temp = zeros(2*Nsc,K,L);
...
phases = rand(K,NOF)*0;
if NOF>1
    % Wide band signal ( BW = (0.4*Fs-max_dop))
    f1 = (max_dop:(0.4*Fs-max_dop)/(NOF-1):0.4*Fs);
...
% window=ones(Ns,1);
% p=10;
% window(1:p)=zeros(p,1);
% window(end-p+1:end)=zeros(p,1);
b_vec=ones(1,K,L);
...
for snr_ind = 1:length(SNRdB)
    ...
    for nexp = 1:Nexp
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Create the obsreved signal at the nth Expriment
        %    r_ell_k = b_ell_k *S_k(t - TOA_ell_k) * exp[j*2*pi*(Ni_k+fc*mu_ell_k)*(t - TOA_ell_k)] + w_ell_k;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %EmitterPos=EmitterPos+(rand(size(EmitterPos))-.5);
         for k = 1:K
             for ell = 1:L

                [TOA,Doppler_shift] = Create_Tau_Doppler(EmitterPos,ReceiverPos,ReceiverVel,fc,c,L,K);
                reg_sig(:,k) = amplitudes(k,:)*sin( 2*pi*(f1')*time_vec ) ; 
                s1=floor(TOA(ell,k)/Tsc);
                s2=ceil(TOA(ell,k)/Tsc);
                T1=TOA(ell,k)-s1*Tsc;
                T2=Tsc-T1;
                int=circshift(reg_sig(:,k),s1)*T2/Tsc + circshift(reg_sig(:,k),s2)*T1/Tsc;
                %int=circshift(reg_sig(:,k),s1);
                Delayed_sig(:,k,ell)=int.*exp(i*2*pi*Doppler_shift(ell,k)*time_vec');
                E_sig = Delayed_sig(:,k,ell)'*Delayed_sig(:,k,ell);
                noise = sqrt(E_sig*sigma2/2)*(randn(Nsc,1)+i*randn(Nsc,1));
...
%                     r_vec_t(:,k,ell) = Delayed_sig  ;  
        ...
        if DPDRun
            ...
            while dLR_DPD > .1
                counter = counter+1;
                if CF1
                    Xsearch_DPD = [-50:1:50]+EmitterPos(1);
                    Ysearch_DPD = [-50:1:50]+EmitterPos(2);
                end
                
                [EmitPosEst_DPD,Xest_DPD,Yest_DPD,CF_DPD]  = DPD(time_vec,Xsearch_DPD,Ysearch_DPD,ReceiverPos,ReceiverVel,K,L,...
...
%             end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % CONVENTIONAL EMITTER LOCALIZATION
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
